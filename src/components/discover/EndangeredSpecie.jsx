...
import EndangeredSpecieForm from './EndangeredSpecieForm'

const EndangeredSpecie = () => {
  ...
  const addOrEdit = (obj) => {
    if (currentId == '')
      firebaseDb.ref('discover/fishingregulations/endangeredspecies').push(obj, (err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
    else
      firebaseDb.ref(`discover/fishingregulations/endangeredspecies/${currentId}`).set(obj, (err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
  }
  ...
  const onDelete = (key) => {
    if (window.confirm('Are you sure to delete this record?')) {
      firebaseDb.ref(`discover/fishingregulations/endangeredspecies/${key}`).remove((err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
    }
        ...
        <div id='content-wrapper' className='d-flex flex-column'>
          ...
          <div id='content'>
            ...
            <div className='container-fluid'>
              ...
              <p className='mb-4'>Data entry for fishing regulations for the list of fish that are not allowed to be caught.</p>
              ...
              <EndangeredSpecieForm {...{ addOrEdit, currentId, contactObjects }} />
              {/* DataTales Example  */}
              <div className='card shadow mb-4'>
                <div className='card-header py-3'>
                  <h6 className='m-0 font-weight-bold text-primary'>
                    Discover - Fihsing Regulations - Endangered Species
                  </h6>
                </div>
...
export default EndangeredSpecie