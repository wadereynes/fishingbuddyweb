...
import CatchSizeRulesForm from './CatchSizeRulesForm'
import BfarRegulationsForm from './BfarRegulationsForm'
...
const BfarRegulations = () => {
  ...
  const addOrEdit = (obj) => {
    if (currentId == '')
      firebaseDb.ref('discover/fishingregulations/bfarregulations').push(obj, (err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
    else
      firebaseDb.ref(`discover/fishingregulations/bfarregulations/${currentId}`).set(obj, (err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
  }
  ...
  const onDelete = (key) => {
    if (window.confirm('Are you sure to delete this record?')) {
      firebaseDb.ref(`discover/fishingregulations/bfarregulations/${key}`).remove((err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
    }
        ...
        <div id='content-wrapper' className='d-flex flex-column'>
          ...
          <div id='content'>
            ...
            <div className='container-fluid'>
              {/* Page Heading  */}
              <h1 className='h3 mb-2 text-gray-800'>BFAR Fishing Regulations</h1>
              <p className='mb-4'>Data entry for fishing regulations set by Bureau of Fisheries and Aquatic Resources</p>
              <Link to='/fishingregulations' class='btn btn-primary btn-icon-split btn-sm mb-3'>
                <span class='icon text-white-50'>
                  <i class='fas fa-arrow-right'></i>
                </span>
                <span class='text'>Back to Fishing Regulations</span>
              </Link>
              <BfarRegulationsForm {...{ addOrEdit, currentId, contactObjects }} />
              {/* DataTales Example  */}
              <div className='card shadow mb-4'>
                <div className='card-header py-3'>
                  <h6 className='m-0 font-weight-bold text-primary'>
                    Discover - BFAR Fishing Regulations 
                  </h6>
                </div>
                        ...
                        {Object.keys(contactObjects).map((id) => {
                          return (
                            <tr key={id}>
                              ...
                              <td>{id}</td>
                              <td>{contactObjects[id].regulationName}</td>
                              <td>{contactObjects[id].regulationDescription}</td>
                              <td>{contactObjects[id].source}</td>
                              <td>
...
export default BfarRegulations