...
const FishingHotspots = () => {
  ...
  const addOrEdit = (obj) => {
    if (currentId == '')
      firebaseDb.ref('discover/fishinghotspots').push(obj, (err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
    else
      firebaseDb.ref(`discover/fishinghotspots/${currentId}`).set(obj, (err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
  }
  ...
  const onDelete = (key) => {
    if (window.confirm('Are you sure to delete this record?')) {
      firebaseDb.ref(`discover/fishinghotspots/${key}`).remove((err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
    }
...
export default FishingHotspots