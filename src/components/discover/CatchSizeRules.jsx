...
const CatchSizeRules = () => {
  ...
  useEffect(() => {
    firebaseDb.ref('discover/fishingregulations/catchsizerules').on('value', (snapshot) => {
  ...
  const addOrEdit = (obj) => {
    if (currentId == '')
      firebaseDb.ref('discover/fishingregulations/catchsizerules').push(obj, (err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
    else
      firebaseDb.ref(`discover/fishingregulations/catchsizerules/${currentId}`).set(obj, (err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
  }
  ...
  const onDelete = (key) => {
    if (window.confirm('Are you sure to delete this record?')) {
      firebaseDb.ref(`discover/fishingregulations/catchsizerules/${key}`).remove((err) => {
        if (err) console.log(err)
        else setCurrentId('')
      })
    }
        ...
        <div id='content-wrapper' className='d-flex flex-column'>
          ...
          <div id='content'>
            ...
            <div className='container-fluid'>
              {/* Page Heading  */}
              <h1 className='h3 mb-2 text-gray-800'>Fishing Regulations</h1>
              <p className='mb-4'>Data entry for fishing regulations regarding what's the size of fish that will be released back.</p>
              ...
              <CatchSizeRulesForm {...{ addOrEdit, currentId, contactObjects }} />
              {/* DataTales Example  */}
              <div className='card shadow mb-4'>
                <div className='card-header py-3'>
                  <h6 className='m-0 font-weight-bold text-primary'>
                    Discover - Fishing Regulations - Catch Size Rules
                  </h6>
                </div>
                        ...
                        {Object.keys(contactObjects).map((id) => {
                          return (
                            <tr key={id}>
                              ...
                              <td>{contactObjects[id].fishSize}</td>
                              <td>
