...
function CreateHobbyList({ user }) {
  const [hobbylist, setList] = React.useState(DEFAULT_LIST)
  ...
  async function handleCreateList() {
    try {
      setSubmitting(true)
      await db.createHobbyList(hobbylist, user)
      mutate(user.uid)
  ...
  return (
    <div className='flex flex-col text-center w-full mb-12'>
      ...
      <div className='lg:w-2/6 mx-auto md:w-1/2 bg-gray-800 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0'>
        ...
        />
        {hobbylist.image && (
          <img className='mb-4' src={URL.createObjectURL(hobbylist.image)} />
        )}
        <button
...
export default CreateHobbyList
